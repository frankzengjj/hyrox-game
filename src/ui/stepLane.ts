...
export interface StepLaneLabels {
  L: string;
  R: string;
  caption: string;
}

export const FOOT_LABELS: StepLaneLabels = {
  L: 'L  A ←',
  R: 'R  D →',
  caption: 'step on the beat: left foot = A / ← / left click · right foot = D / → / right click',
};

/** Two-row tap highway: left foot (or hand) on top, right below. */
export class StepLane {
  ...
  constructor(
    ...
    lookaheadMs: number,
    labels: StepLaneLabels = FOOT_LABELS,
  ) {
    ...
    scene.add.text(LEFT + 8, ROW_Y.L, labels.L, textStyle(10, '#9fd3ff')).setOrigin(0, 0.5);
    scene.add.text(LEFT + 8, ROW_Y.R, labels.R, textStyle(10, '#ffe680')).setOrigin(0, 0.5);
    scene.add.text(LEFT + 4, Y - H / 2 - 14, labels.caption, textStyle(11, '#c9ccd2')).setOrigin(0, 0);
    this.g = scene.add.graphics();
    this.status = scene.add.text((HIT_X + RIGHT) / 2, Y, '', textStyle(15, '#ffffff', { fontStyle: 'bold' })).setOrigin(0.5);
    this.count = scene.add.text(HIT_X + 60, Y, '', textStyle(26, '#ffd400', { fontStyle: 'bold' })).setOrigin(0.5);
  }
  ...
  render(now: number, track: StepTrack, lactate: number): void {
    ...
    this.status
      .setText(track.state === 'resting' ? 'tap left or right to start a set' : '')
      .setAlpha(0.55 + 0.45 * Math.sin(now / 180));
  }
